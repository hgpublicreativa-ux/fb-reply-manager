      ...
      <main className="max-w-4xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
        ...
        <div className="flex items-center justify-between gap-3 mb-4 sm:mb-6">
          <div className="min-w-0">
            <h1 className="text-lg sm:text-2xl font-bold text-gray-900">Mis Cuentas</h1>
            <p className="text-gray-500 text-xs sm:text-sm mt-0.5">
              {accounts.length} de {MAX_ACCOUNTS} conectadas
            </p>
          </div>
          <button
            onClick={handleConnectFacebook}
            disabled={connecting || accounts.length >= MAX_ACCOUNTS}
            className="btn-primary flex items-center gap-1.5 disabled:opacity-50 text-sm px-3 py-2 flex-shrink-0"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
            <span className="hidden xs:inline sm:inline">{connecting ? 'Conectando...' : 'Conectar Facebook'}</span>
            <span className="xs:hidden sm:hidden">{connecting ? '...' : '+ FB'}</span>
          </button>
        </div>
              ...
              <div key={account.id} className="card p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0 flex-wrap justify-end">
                  <Link
                    ...
                    className="btn-secondary text-xs sm:text-sm px-2.5 sm:px-3 py-1.5 flex items-center gap-1"
                  >
                  ...
                  {accounts.length > 1 && (
                    <button
                      ...
                      className="text-xs sm:text-sm text-red-500 hover:text-red-700 hover:bg-red-50 px-2.5 sm:px-3 py-1.5 rounded-lg border border-red-200 transition-colors flex items-center gap-1"
                    >
                      <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
